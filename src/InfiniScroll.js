...
class InfiniItem extends React.Component {
  render() {
    return (
      <div ref="root" style={{height: Math.abs(this.props.i)}}>
        hi {this.props.i}
      </div>
    );
  }
...
export default class InfiniScroll extends React.Component {
  ...
  render() {
    ...
    return (
      <div style={{background: 'red', height: '100%'}}>
        {_.range(this.state.start, this.state.end).map(i => (
          <InfiniItem
            i={i}
            hasHeight={h => {
              if (h !== this.state.heights[i]) {
                const heights = Object.assign({}, this.state.heights, {
                  [i]: h
                });
  ...
  componentWillUpdate() {
    ...
    for (
      i = this.start;
      typeof this.state.heights[i] === 'number' && sum < scrollY;
      ++i
    ) {
