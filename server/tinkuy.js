...
async function tinkuySync(mycrud) {
  ...
  try {
    ...
    eventsRaw = await (await fetch(
      'https://www.tinkuy.dk/events.json'
    )).json();
  }
  ...
  for (const event of events) {
    let prevEvent = JSON.parse(
      (await mycrud.get('tinkuy', 'events', event._id, '{}')).data
    );
    for (k in event) {
