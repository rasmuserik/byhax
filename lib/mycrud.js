...
async function getVersionData(owner, db, key) {
  ...
  return [value.slice(0, 23), value.slice(23)];
}
...
async function put(owner, db, key, value, opt) {
  ...
  let newVersion = new Date().toISOString().slice(0, 23);
...
router.get('/:owner/:db', async (req, res) => {
  const [p, q] = [req.params, req.query];
  if (q.gt && q.lt) {
    res.set('Content-Type', 'application/cbor');
    const lt = [p.owner, p.db, q.lt].join('/');
    const gt = [p.owner, p.db, q.gt].join('/');
    const dbNameLength = p.owner.length + p.db.length + 2;
    const stream = mcData
  ...
  if (q.since) {
    ...
    const ownerDb = `${p.owner}/${p.db}/`;
    const gt = ownerDb + q.since;
    ...
    const stream = mcVersion
      ...
      .on('data', async data => {
        ...
        try {
          writing = true;
          value = await mcData.get(ownerDb + key);
          writing = false;
        } catch (e) {
          writing = false;
          console.log('ERROR', e);
